<?php
    ...
    try {
        ...
        $roleStmt = $conn->prepare("SELECT role FROM [LRNPH_OJT].[dbo].[acts_restrictions] WHERE TRY_CAST(biometrics_id AS NVARCHAR(50)) = ?");
        ...
        if ($action === 'add') {
            ...
            $checkStmt = $conn->prepare("SELECT id FROM [LRNPH_OJT].[dbo].[acts_restrictions] WHERE TRY_CAST(biometrics_id AS NVARCHAR(50)) = ?");
            ...
            $insertStmt = $conn->prepare("INSERT INTO [LRNPH_OJT].[dbo].[acts_restrictions] (biometrics_id, role) VALUES (?, ?)");
        ...
        if ($action === 'update_role') {
            ...
            $updateStmt = $conn->prepare("UPDATE [LRNPH_OJT].[dbo].[acts_restrictions] SET role = ? WHERE id = ?");
        ...
        if ($action === 'delete') {
            ...
            // Get target
            $targetStmt = $conn->prepare("SELECT id, role, biometrics_id FROM [LRNPH_OJT].[dbo].[acts_restrictions] WHERE id = ?");
            $targetStmt->execute([$targetId]);
            $target = $targetStmt->fetch(PDO::FETCH_ASSOC);
            ...
            $deleteStmt = $conn->prepare("DELETE FROM [LRNPH_OJT].[dbo].[acts_restrictions] WHERE id = ?");
        ...
        if ($action === 'transfer') {
            ...
            $demoteStmt = $conn->prepare("UPDATE [LRNPH_OJT].[dbo].[acts_restrictions] SET role = 'admin' WHERE TRY_CAST(biometrics_id AS NVARCHAR(50)) = ?");
            $demoteStmt->execute([$myEmpId]);

            // Promote target to super_admin
            $promoteStmt = $conn->prepare("UPDATE [LRNPH_OJT].[dbo].[acts_restrictions] SET role = 'super_admin' WHERE id = ?");
